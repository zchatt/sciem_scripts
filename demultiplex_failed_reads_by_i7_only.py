...
with gzip.open(read_file, "rt") as inf:
    with gzip.open("n9_failed_reads_100k.fq.gz", "wt") as n9_wf:
        with gzip.open("mg_failed_reads_100k.fq.gz", "wt") as mg_wf:
            with gzip.open("em_failed_reads_100k.fq.gz", "wt") as em_wf:
                with gzip.open("not_allocated_failed_reads_100k.fq.gz", "wt") as non_wf:
                    ...
                    while True:
                        ...
                        for key, value in lib_i7.items():
                            hd = levenshtein.hamming(barcode, value)
                            hd_dict[key] = hd
#                         print(hd_dict)
                        if min(hd_dict.values()) <= 2:
                            ...
                            non_wf.write(first_line + read + sep + qual)

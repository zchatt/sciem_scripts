...
with open(sys.argv[1]) as csv_file:
    ...
    for row in csv_reader:
        if row[3].startswith("sciMET_i7_7"):
            cluster_dict[row[0]] = "1" ## this is for mg primer
        elif (row[3].startswith("sciMET_i7_8") or row[2].startswith("sciMET_i7_9")):
              cluster_dict[row[0]] = "2" ## this is for n9 primer
...
for cluster in clusters:
    fout = pysam.AlignmentFile("cluster_em" + cluster + ".bam", "wb", template = fin)
